...
//Mf calculation loop 2
  ...
  package SampleSimulations
    ...
    model SeaSurfaceElevation
      Components.Waves.IrregularWave.IRW_PM_RDFCWI irw_pm_rdfcwi1(Hs = 1, d = 50) annotation(
        Placement(visible = true, transformation(origin = {0, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      annotation(
        experiment(StartTime = 0, StopTime = 600, Tolerance = 1e-06, Interval = 0.5));
    end SeaSurfaceElevation;


    model Fig4_SimulationOfHeave
      constant Real g = Modelica.Constants.g_n "Value of acceleration due to gravity";
      constant Real pi = Modelica.Constants.pi "Value of pi";
      parameter Real d = 100;
      parameter Real rho_w = 1025;
      parameter Modelica.SIunits.Length r = 0.6 "Radius of the buoy";
      parameter Modelica.SIunits.Length h = 1 "Height of the buoy";
      parameter Modelica.SIunits.Mass mbu = 1000 "Structural mass of the buoy";
      parameter Modelica.SIunits.Mass mba = 0 "Ballast mass";
      parameter Real Cma = 0.8 "Added mass coefficient";
      parameter Real Cd_buoy = 1 "Drag coefficient of buoy";
      parameter Modelica.SIunits.Mass M = (mbu + mba) * (1 + Cma) "Effective mass of body including added mass";
      parameter Modelica.SIunits.Length KGb = 1.25 "KG of buoy";
      parameter Real zeta = 0.1 "Damping factor";
      parameter Real K(unit = "N m^-1") = rho_w * g * pi * r ^ 2 "Coefficient of Stiffness";
      parameter Real C(unit = "kg/s") = 2 * zeta * (K * M) ^ 0.5 "Coefficient of damping";
      parameter Modelica.SIunits.Length z_s = (mbu + mba) / (pi * r ^ 2 * rho_w) "Static draught";
      parameter Modelica.SIunits.Length z_fb = KGb - z_s "VCG of body wrt SWL";
      parameter Modelica.SIunits.Length pz = 0.5 "Depth of panel to calculate current force & Morison Force";
      parameter Integer CMf_b = 0;
      parameter Real zinit = 0;
      Real a(start = 0, fixed = true) "Wave Amplitude";
      Modelica.SIunits.Length SSE_X "Sea surface elevation corresponding to buoy CG X coordinate";
      Modelica.SIunits.Length z "Displacement of body VCG from z_fb";
      Modelica.SIunits.Velocity v_z "Velocity of body VCG in vertical direction";
      Modelica.SIunits.Length z1 "Position of body VCG";
    ...
    equation
      ...
      elseif time < 40 then
        ...
        der(a) = if noEvent(a > 0) then -1 / 20 else 0;
      end if;
      SSE_X = a * 0.5 * sin(2 * pi / 3 * time);
      v_z = der(z);
      M * der(v_z) + C/2 * der(z) + K * z = rho_w * g * pi * r ^ 2 * (SSE_X - z3) - (mbu + mba) * g;
      z1 = z_fb + z;
      z2 = z1 + h - KGb;
      z3 = z1 - KGb;
      annotation(
    ...
    end Fig4_SimulationOfHeave;



    package FloatingCylinderInWavesAndOrCurrent
      model UnmooredCydlBuoy
        extends Modelica.Blocks.Icons.Block;
        Modelica.Mechanics.Translational.Interfaces.Flange_a TopHook(s = z2) "Hook for spring" annotation(
          Placement(transformation(extent = {{-20, 120}, {20, 80}})));
        Modelica.Mechanics.Translational.Interfaces.Flange_a Fairlead[2](s = {x, z3}) annotation(
          Placement(transformation(extent = {{-20, -120}, {20, -80}})));
        OceanEngineering.SampleSimulations.FloatingCylinderInWavesAndOrCurrent.Ebdc ebdc(n_omega_i = nOmega) annotation(
          Placement(visible = true, transformation(extent = {{-80, -20}, {-120, 20}}, rotation = 0), iconTransformation(extent = {{-118, -20}, {-78, 20}}, rotation = 0)));
        ...
        parameter Modelica.SIunits.Length h = 2 "Height of the buoy";
        parameter Modelica.SIunits.Mass mbu = 350 "Structural mass of the buoy";
        parameter Modelica.SIunits.Mass mba = 500 "Ballast mass";
        parameter Real Cma = 1 "Added mass coefficient";
        parameter Real Cd_buoy = 1 "Drag coefficient of buoy";
        parameter Modelica.SIunits.Mass M = (mbu + mba) * (1 + Cma) "Effective mass of body including added mass";
        parameter Modelica.SIunits.Mass M1=mbu+mba;
        parameter Modelica.SIunits.Length KGb = 1.25 "KG of buoy";
      ...
      equation
        ...
        u_x = der(x);
        a_x = der(u_x);
        (M1+Cma*pi*r^2*rho_w*(SSE_X-z3)) * der(u_x)+0*(C/2*u_x)*(SSE_X-z3) = Fairlead[1].f + Mf;
        annotation(
          Icon(graphics = {Rectangle(origin = {0, -50}, fillColor = {85, 170, 255}, fillPattern = FillPattern.Solid, extent = {{-100, 50}, {100, -50}}), Rectangle(origin = {0, 3}, fillColor = {255, 85, 0}, fillPattern = FillPattern.Solid, extent = {{-28, 53}, {28, -53}}), Ellipse(origin = {0, 57}, fillColor = {255, 85, 0}, fillPattern = FillPattern.Solid, extent = {{-28, 5}, {28, -5}}, endAngle = 360), Ellipse(origin = {0, -51}, fillColor = {255, 85, 0}, fillPattern = FillPattern.Solid, extent = {{-28, 5}, {28, -5}}, endAngle = 360)}, coordinateSystem(initialScale = 0.1)),
          experiment(StartTime = 0, StopTime = 300, Tolerance = 1e-06, Interval = 0.5));
      end UnmooredCydlBuoy;
      ...
      model FloatingCylinderInWavesAndCurrent
        OceanEngineering.SampleSimulations.FloatingCylinderInWavesAndOrCurrent.UnmooredCydlBuoy unmooredCydlBuoy_W(d = 50) annotation(
          Placement(visible = true, transformation(origin = {50, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      ...
      equation
        ...
        annotation(
          Line(points = {{-40, 30}, {-20, 30}, {-20, 0}, {40, 0}, {40, 0}}),
          experiment(StartTime = 0, StopTime = 80, Tolerance = 1e-06, Interval = 0.5));
      end FloatingCylinderInWavesAndCurrent;




    end FloatingCylinderInWavesAndOrCurrent;
    ...
    package Fig_9And10_MorisonLoadOnMooring
      ...
      model RW_ResponseBuoy_MfC
        ...
        Components.Floaters.CylindricalBuoy cylindricalBuoy1(nOmega = 1, d = 50, mba=750,spm_chain_sub=14) annotation(
          Placement(visible = true, transformation(origin = {30, 30}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
        Components.Moorings.CatenaryMooring_MfC catenaryMooring_MfC1(nOmega = 1, d = 50, n_lnk = 50, lnk_l = 2, d_chain = 0.028, spm_chain = 16, spm_chain_sub=14,Trmp=200) annotation(
          Placement(visible = true, transformation(origin = {30, -30}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      equation
        connect(catenaryMooring_MfC1.shackle, cylindricalBuoy1.Fairlead) annotation(
          Line(points = {{30, -20}, {30, -20}, {30, 20}, {30, 20}}, color = {0, 127, 0}, thickness = 0.5));
    ...
    end Fig_9And10_MorisonLoadOnMooring;


    package Fig11_EffectOfSpringMass
      model UnmooredCydlBuoy_NoWave_NoCurrent
        extends Modelica.Blocks.Icons.Block;
        Modelica.Mechanics.Translational.Interfaces.Flange_a TopHook(s = z2) "Hook for spring" annotation(
          Placement(transformation(extent = {{-20, 120}, {20, 80}})));
        parameter Integer nOmega = 1 "Number of wave components; 1 for regular wave and more than 1 for irregular wave";
      ...
      equation
        SSE_X = 0;
        v_z = der(z);
        M* der(v_z) + C/2 * der(z)*(SSE_X-z3)+ K * z = rho_w * g * pi * r ^ 2 * SSE_X+ TopHook.f;
        z1 = z_fb + z;
        z2 = z1 + h - KGb;
        z3 = z1 - KGb;
        Mf = 0;
        u_x = der(x);
        a_x = der(u_x);
         (M1+Cma*pi*r^2*rho_w*(SSE_X-z3)) * der(u_x)+0*(C/2*u_x)*(SSE_X-z3) = Mf;
        annotation(
          Icon(graphics = {Rectangle(origin = {0, -50}, fillColor = {85, 170, 255}, fillPattern = FillPattern.Solid, extent = {{-100, 50}, {100, -50}}), Rectangle(origin = {0, 3}, fillColor = {255, 85, 0}, fillPattern = FillPattern.Solid, extent = {{-28, 53}, {28, -53}}), Ellipse(origin = {0, 57}, fillColor = {255, 85, 0}, fillPattern = FillPattern.Solid, extent = {{-28, 5}, {28, -5}}, endAngle = 360), Ellipse(origin = {0, -51}, fillColor = {255, 85, 0}, fillPattern = FillPattern.Solid, extent = {{-28, 5}, {28, -5}}, endAngle = 360)}, coordinateSystem(initialScale = 0.1)),
          experiment(StartTime = 0, StopTime = 50, Tolerance = 1e-06, Interval = 0.5));
      end UnmooredCydlBuoy_NoWave_NoCurrent;
      ...
      model EffectsOfSpringMass_SmallInitialDisp
        UnmooredCydlBuoy_NoWave_NoCurrent unmooredCydlBuoy_NoWave_NoCurrent1 annotation(
          Placement(visible = true, transformation(origin = {0, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
        MiscellaneousComponents.DampedSpring_Vertical dampedSpring_Vertical1 annotation(
          Placement(visible = true, transformation(origin = {40, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
        MiscellaneousComponents.SuspendedMass suspendedMass1(s(start = 0.1), m = 100) annotation(
          Placement(visible = true, transformation(origin = {40, -40}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      equation
        connect(dampedSpring_Vertical1.flange_a, suspendedMass1.flange_b) annotation(
          Line(points = {{40, -10}, {40, -10}, {40, -32}, {40, -32}}, color = {0, 127, 0}));
        ...
        annotation(
          experiment(StartTime = 0, StopTime = 50, Tolerance = 1e-06, Interval = 0.05));
      end EffectsOfSpringMass_SmallInitialDisp;
    end Fig11_EffectOfSpringMass;
    ...
    model HeaveSurgeResponseOfCydlBuoy
     OceanEngineering.SampleSimulations.FloatingCylinderInWavesAndOrCurrent.UnmooredCydlBuoy unmooredCydlBuoy_W annotation(
        Placement(visible = true, transformation(origin = {50, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      OceanEngineering.Components.Waves.RegularWave.Regular_Airy_Wave regular_Airy_Wave2(d = 50, Hr = 1,Tr=7, Trmp=20) annotation(
        Placement(visible = true, transformation(origin = {-50, 30}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Components.CurrentProfiles.CurrentProfile_4pt currentProfile_4pt2(zcg = {-50, -25, -10, 0}, Uf = {0, 0, 0, 0}, Trmp=20) annotation(
        Placement(visible = true, transformation(origin = {-50, -30}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    equation
      connect(regular_Airy_Wave2.wdoc.omega, unmooredCydlBuoy_W.ebdc.omega) annotation(
        Line(points = {{-40, 30}, {-20, 30}, {-20, 0}, {40, 0}, {40, 0}}));
    ...
    end HeaveSurgeResponseOfCydlBuoy;


    package MorisonLoadsOnFixedCydl
      model FixedCydl
        extends Modelica.Blocks.Icons.Block;
        Modelica.Mechanics.Translational.Interfaces.Flange_a TopHook(s = z2) "Hook for spring" annotation(
          Placement(transformation(extent = {{-20, 120}, {20, 80}})));
        ...
        parameter Modelica.SIunits.Length z_s = (mbu + mba) / (pi * r ^ 2 * rho_w) "Static draught";
        parameter Modelica.SIunits.Length z_fb = KGb - z_s "VCG of body wrt SWL";
        parameter Modelica.SIunits.Length pz = 1 "Depth of panel to calculate current force & Morison Force";
        parameter Integer CMf_b = 0;
        Modelica.SIunits.Length SSE_X "Sea surface elevation corresponding to buoy CG X coordinate";
        Modelica.SIunits.Length z "Displacement of body VCG from z_fb";
        Modelica.SIunits.Velocity v_z "Velocity of body VCG in vertical direction";
    ...
    end MorisonLoadsOnFixedCydl;

    model MooredBuoyIn0Current
      OceanEngineering.Components.Waves.RegularWave.Regular_Airy_Wave regular_Airy_Wave1(d = 50, Hr = 0, Tr = 7, Trmp = 50) annotation(
        Placement(visible = true, transformation(origin = {-70, 30}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ...
    equation
      ...
      connect(regular_Airy_Wave1.wdoc.zeta0i, environmentBus1.zeta0i);
      annotation(
        experiment(StartTime = 0, StopTime = 150, Tolerance = 1e-03, Interval = 1));
    end MooredBuoyIn0Current;
